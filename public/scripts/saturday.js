...
// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
const newDay = () => {
  if(23=== date.getHours() && 59 === date.getMinutes() && actualDay === "Saturday"){
...
const displayTask = (data) => {
  data.allTaskSaturday.forEach(el => {
    if(el.todo != null){
    let newTask = `<li class="inputDisplay" data-id=${el.saturday_id}><p>${el.todo}</p><i class="fas fa-times-circle redHover"></i></li>`
    taskInput[0].insertAdjacentHTML("beforeend", newTask)
    // sortable
    let sortTask = document.getElementById("toDoInput")
    let sortable = Sortable.create(sortTask, {animation: 100, ghostClass: 'myghostclass', dragClass: 'sortable-drag',
  ...
  data.allTaskSaturday.forEach(el => {
    if(el.workout != null){
    let newTask = `<li class="inputDisplay" data-id=${el.saturday_id}><p>${el.workout}</p><i class="fas fa-times-circle redHover"></i></li>`
    taskInput[1].insertAdjacentHTML("beforeend", newTask)
    // sortable
    let sortTask = document.getElementById("workoutInput")
    let sortable = Sortable.create(sortTask, {animation: 100, ghostClass: 'myghostclass', dragClass: 'sortable-drag',
  ...
  data.allTaskSaturday.forEach(el => {
    if(el.meal != null){
    let newTask = `<li class="inputDisplay" data-id=${el.saturday_id}><p>${el.meal}</p><i class="fas fa-times-circle redHover"></i></li>`
    taskInput[2].insertAdjacentHTML("beforeend", newTask)
    // sortable
    let sortTask = document.getElementById("mealInput")
    let sortable = Sortable.create(sortTask, {animation: 100, ghostClass: 'myghostclass', dragClass: 'sortable-drag',
  ...
  data.allTaskSaturday.forEach(el => {
    if(el.buy != null){
    let newTask = `<li class="inputDisplay" data-id=${el.saturday_id}><p>${el.buy}</p><i class="fas fa-times-circle redHover"></i></li>`
    taskInput[3].insertAdjacentHTML("beforeend", newTask)
    // sortable
    let sortTask = document.getElementById("buyInput")
    let sortable = Sortable.create(sortTask, {animation: 100, ghostClass: 'myghostclass', dragClass: 'sortable-drag',
  ...
  deleteBtn.forEach(element => {
    element.addEventListener("click", (closeBtn) => {
      ...
      deleteTheTask({table:chooseDay.innerHTML, column:"saturday_id", data: taskRemove.dataset.id})
      taskRemove.remove()
        })
}); 
}
...
const updatePlace = (infos) => {
  ...
  for(let i =0; i<inputDisplay.length; i++){
    fetch('api/updateplace', {
      ...
      body: JSON.stringify({table: "saturday",id: inputDisplay[i].dataset.id, place:i, condition:"saturday_id"}),
    })
    .then(response => response.json())
    .then(data => {
      console.log('Success:', data);
